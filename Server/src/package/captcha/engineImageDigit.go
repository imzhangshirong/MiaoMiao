...
func randomDigits(len int)[]int{
	rand.Seed(time.Now().Unix())
	var digits []int
	for i:=0;i<len;i++ {
		digits = append(digits,rand.Intn(9))
	}
	return digits
}

//EngineDigitsCreate create captcha by engine-digits with configuration.
func EngineDigitsCreate() (*CaptchaImageDigit,string) {
	rand.Seed(time.Now().Unix())
	...
	x := border+rand.Intn(maxx-border)
	y := border+rand.Intn(maxy-border)
	str := ""
	// Draw digits.
	...
	m.distort(5+rand.Float64()*5, 100+rand.Float64()*100)
	// Fill image with random circles.
	m.fillWithCircles(config.DotCount, m.dotSize)
	return m,str
}

func (m *CaptchaImageDigit) getRandomPalette() color.Palette {
	p := make([]color.Color, config.DotCount+1)
	// Transparent color.
...
func (m *CaptchaImageDigit) fillWithCircles(n, maxradius int) {
	...
	maxy := m.Bounds().Max.Y
	for i := 0; i < n; i++ {
		colorIdx := uint8(1+rand.Intn(config.DotCount-1))
		r := 1+rand.Intn(maxradius)
		m.drawCircle(r+rand.Intn(maxx-r), r+rand.Intn(maxy-r), r, colorIdx)
	}
}
...
func (m *CaptchaImageDigit) strikeThrough() {
	maxx := m.Bounds().Max.X
	maxy := m.Bounds().Max.Y
	y := maxy/3 + rand.Intn(maxy-maxy/3)
	amplitude := 5+rand.Float64()*15
	period := 80+rand.Float64()*100
...
func (m *CaptchaImageDigit) drawDigit(digit []byte, x, y int) {
	skf := -config.MaxSkew + rand.Float64()*config.MaxSkew*2
	xs := float64(x)
	r := m.dotSize / 2
	y += -r + rand.Intn( 2*r)
